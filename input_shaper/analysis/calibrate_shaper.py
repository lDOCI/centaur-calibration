...
# Добавляем путь к extras, если он ещё не добавлен
if os.path.join(os.path.dirname(os.path.realpath(__file__)), 'extras') not in sys.path:
    sys.path.append(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'extras'))

# Импортируем shaper_calibrate напрямую
import shaper_calibrate
# Используем абсолютный импорт для shaper_defs
import shaper_defs
...
def plot_freq_response(lognames, calibration_data, shapers,
    ...
    for shaper in shapers:
        label = "%s (%.1f Hz, vibr=%.1f%%, shaper_defs.sm~=%.2f, accel<=%.f)" % (
